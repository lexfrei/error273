...
What goes up is voted on. Each citizen casts one voice for whatever has cost them most since the last ballot -- a house for rest, a hut for hunger, a boiler for cold -- and the highest tally wins, with ties resolved in a fixed order so the same colony always decides the same way. Cost is how far short a need fell, summed over every hour it was short, so a tiredness answered by morning weighs less than a hunger left standing for a week even though both bottom out at the same depth. A fourth entry sits beside the three needs in the same currency: the hours a citizen spent walking back to the fire instead of working. A boiler answers a cost rather than a need, because a hauler always makes it back to the warm, and what the cold takes from them is time. With every bed taken the colony builds a house regardless: that one is not a preference but the precondition for growing at all.
