...
under 15   2  grown  20  over 40  10  couples  12  raised 0.52/0.46/0.53  mood  50  night song boring
...
The arithmetic of a chain has now been done, and it says the colony should not build one. What a lit chain costs is four wood a day per post, burning whether anybody is out there or not; what it buys is what one hauler gains a day by making the same trip warm rather than cold, which is a fraction of a unit, because a far trip is already several times worse an hour than the near ring and taking the cold out of it improves one term by about a third. Divide the two and the answer is a number of haulers who would have to be working past the last link for the thing to break even: **ninety-six for a corridor thirty cells long, and two hundred and ninety-four for one reaching the far edge**, against a colony founded with thirty people that has never held more than fifty-six. A hotter brazier shortens the chain and does not rescue it.

The two numbers the colony builds with do not describe a chain either. A post is sited up to thirty-seven cells from the warmth that feeds it and its own warm disc is four and a half, so what gets built is a string of separate braziers with open cold between them. Closing that by output alone would want a brazier brighter than the generator, which is not a brazier. None of it was tuned: moving a number cannot make a corridor pay, and a changed constant with nothing behind it is worse than an honest finding.
...
What is further out is worth more, and a trip out there brings home more for it. A patch's cap is flat out to the old rim and rises past it, until about a hundred and eighty cells out where it stops; the same rise is a factor on what a citizen lifts, so richer ground fills the sack faster as well as holding more of it. The factor is the ground's, never the walk's -- paying for distance itself would pay a citizen to leave good ground near the hearth for poor ground further out, which is not a thing a forager does. Far ground pays here because far ground is richer. Inside the old rim the factor is exactly one, so the near ring is the trip it always was.

A citizen looking for work takes the near ring first and by distance alone, exactly as the colony always did: the twenty-four cells around the hearth are its whole diet and nothing about richer ground is allowed to move them. Only when nothing of that kind is standing inside the rim does the question change, and then it is not "which is nearest" but "which is worth the most an hour" -- the sack that patch would fill, against the walk out, the walk home, the warmth that walk costs, and the hours of work in between. It is the same currency an hour is priced in when deciding whether to stay, so choosing, working and leaving cannot disagree with one another.

There is no term for distance in that and no bonus for walking. Far ground wins only when what it gives beats what getting there costs, and holding the ground equal a longer walk can only lose. What it does produce is the thing the world was built for: once a walk is needed at all, the best patch to walk to is not the first one you come to.
...
Working a patch takes hours, not an instant. The first hour of a visit brings back a full load; every hour after it brings back less, because the easy wood goes first and what is left takes longer to get at -- and a hauler who comes back tomorrow starts on the easy wood again, so ground the colony works every day is not permanently slower. Whether to stay for another hour is the forager's question and it is answered the forager's way: stay while the next hour would beat what the whole trip has averaged so far, counting the walk out and the walk home as time in which nothing is gathered. The currency is warmth rather than food, because an hour is what a colony pays in body heat, and an hour spent where no fire reaches costs double. The consequence is the one the theory is known for and it is the point: a long walk makes the average lower, so a far patch is worth staying at longer than a near one.

A trip is also bounded by what a person can carry, and the sack is bigger the further out it is filled -- exactly one load at the hearth, better than two at the far edge. That is what keeps the near ring the trip it always was: at the hearth the sack fills in the first hour, so there is nothing for the staying rule to decide, and the ground the colony lives off is untouched by any of this.
...
How it goes is worked out from four named terms rather than one number: how many people turned up against a full house of eight, whether the venue was warm enough, what the performer brings, and how much the colony has done this before -- that last is a seam with nothing behind it yet and it contributes nothing. Quality then decides the odds across four outcomes, not the outcome: a bad night stays possible with a good performer and a full house, and a great one stays possible without them. An evening is a song or a story, and the performer does whichever the colony has heard less of lately. Hearing the same kind again takes some of what the night is worth -- nothing while it is still fresh, up to two thirds of it once the colony is thoroughly tired of it, and never all of it, because a night worth nothing at all is a night nobody would sit through and the mechanic would be a chore rather than a pressure. How fast a colony tires is set by how well it is doing: one at its target stores tires at the full rate, one at half of them at half, and one with nothing will take the same song every night and be glad of it. Measured on a colony that had everything, two kinds are not enough to carry a performance every night -- it loses about half of every evening -- and that is the pressure working rather than failing: the answer to it is more kinds, which is what the culture layer is for.

A terrible or boring night pays nothing. A good one lifts everybody present out of their boredom and leaves them cheered for days, and it leaves the whole colony working a tenth better for as long as that lasts -- the audience is who enjoyed it, the colony is who is carried by it. A second good night inside that window pushes the end of it further out and never makes the lift bigger, because otherwise a colony with two entertainers would simply hold performances until the multiplier paid for everything.
