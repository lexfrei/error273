...
Recreation is the odd one. It slips by 4 a day like the rest and it acts below 35, but there is nowhere to go about it: standing anywhere at all recovers none of it, and no building on the list satisfies it. A citizen therefore never walks to it and never votes for it, which is what keeps a need with no remedy from turning the ballot into a demand for a thing that does not exist. The only thing that fills it is being in the audience when somebody performs.
